<template>
  <div class="min-h-screen bg-slate-950 text-slate-100">
    <div class="mx-auto w-full max-w-4xl px-4 py-10 sm:px-6">
      <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-semibold tracking-tight">Публичная оферта</h1>
        <p class="mt-2 text-sm text-slate-300">
          Договор информационно-консультативных услуг · актуальная редакция
        </p>
        <div v-if="siteUrl" class="mt-2 text-xs text-slate-400">
          Сайт: <span class="text-slate-200">{{ siteUrl }}</span>
        </div>
      </div>

      <div class="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-6">
        <!-- Notion-like: читаемый текст, без “prose” плагина -->
        <div class="whitespace-pre-wrap text-[13px] sm:text-sm leading-6 text-slate-200">
{{ offer }}
        </div>
      </div>
    </div>
...
useSeoMeta({
  title: 'Публичная оферта — Happy',
  description: 'Публичная оферта о заключении договора информационно-консультативных услуг.',
})

const cfg = useRuntimeConfig()

const baseDomain = String((cfg.public as any)?.baseDomain || '').trim()
const siteUrl = computed(() => (baseDomain ? `https://${baseDomain}` : ''))

// Текст оферты (из твоего файла). При желании можешь автоматически подставить siteUrl в “_____”.
const offer = computed(() => {
  const raw = `ПУБЛИЧНАЯ ОФЕРТА
...
Сайт Исполнителя в сети «Интернет» – совокупность программ для электронных вычислительных машин и иной информации, содержащейся в информационной системе, доступ к которой обеспечивается посредством сети «Интернет» по доменному имени и сетевому адресу: https://happy4u.online/
...
Услуга – информационно-консультативные услуги, оказываемые Исполнителем Заказчику в порядке и на условиях, установленных настоящей Офертой.
    ...
    2. Предмет Договора
        ...
        2.2. Наименование, количество, порядок и иные условия оказания Услуг определяются на основании сведений Исполнителя при оформлении заявки Заказчиком, либо устанавливаются на сайте Исполнителя в сети «Интернет» https://happy4u.online/
...
Данный перечень не является исчерпывающим, могут быть и другие действия, которые ясно выражают намерение лица принять предложение контрагента.
    ...
    4. Цена и порядок расчетов
        4.1. Стоимость, а также порядок оказания информационно-консультативных услуг определяется на основании сведений Исполнителя при оформлении заявки Заказчиком, либо устанавливаются на сайте Исполнителя в сети «Интернет»: ____
...
Полное наименование: ОБЩЕСТВО С ОГРАНИЧЕННОЙ ОТВЕТСТВЕННОСТЬЮ "УНИКОРН СТАР"
ИНН: 1103048119
ОГРН/ОГРНИП: 1251100003934
Контактный телефон: +7 912 563-33-89
Контактный e-mail: tumyp2003map@mail.ru
`

  // Небольшая автоподстановка домена, если он есть
  const url = siteUrl.value
  if (!url) return raw
  return raw
    .replace('по доменному имени и сетевому адресу: __________', `по доменному имени и сетевому адресу: ${url}`)
    .replace('в сети «Интернет»______', `в сети «Интернет» ${url}`)
    .replace('в сети «Интернет»: ____', `в сети «Интернет»: ${url}`)
})
</script>
