<template>
  <div>
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold">Пользователи</h1>
      <NuxtLink to="/admin" class="text-sm text-slate-300 hover:text-white">← назад</NuxtLink>
    </div>

    <div class="mt-6 overflow-x-auto rounded-3xl border border-white/10">
      <table class="min-w-full text-sm">
        <thead class="bg-white/5 text-slate-300">
          <tr>
            <th class="text-left px-4 py-3">ID</th>
            <th class="text-left px-4 py-3">Email</th>
            <th class="text-left px-4 py-3">Username</th>
            <th class="text-left px-4 py-3">Role</th>
            <th class="text-left px-4 py-3"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="u in items" :key="u.id" class="border-t border-white/10">
            <td class="px-4 py-3 font-mono text-xs">{{ u.id }}</td>
            <td class="px-4 py-3">{{ u.email }}</td>
            <td class="px-4 py-3">{{ u.username }}</td>
            <td class="px-4 py-3">{{ u.role }}</td>
            <td class="px-4 py-3">
              <button class="px-3 py-1.5 rounded-xl bg-rose-500/10 border border-rose-500/20 hover:bg-rose-500/15 text-rose-200" @click="del(u.id)">
                Удалить
              </button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>

    <div v-if="err" class="mt-4 text-sm text-rose-300">{{ err }}</div>
  </div>
</template>

<script setup lang="ts">
definePageMeta({ middleware: ['admin'] })
const api = useApi()
const items = ref<any[]>([])
const err = ref('')

async function load() {
  const res: any = await api.get('/admin/users')
  items.value = res.items || []
}
onMounted(load)

async function del(id: string) {
  if (!confirm('Удалить пользователя?')) return
  try {
    const res: any = await api.del(`/admin/users/${id}`)
    if (res.ok) await load()
  } catch {
    err.value = 'DELETE_FAILED'
  }
}
</script>
